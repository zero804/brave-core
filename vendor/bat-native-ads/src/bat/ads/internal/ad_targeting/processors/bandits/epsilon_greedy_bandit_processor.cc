...
// TODO(Moritz Haller): Write description in headerfile
...
void EpsilonGreedyBandit::Process(
    const AdEvent& ad_event) {
  // TODO(Moritz Haller): extract top level segment
  std::string segment = ad_event.ad.segment;
  if (ad_event.type == ads::AdNotificationEventType::kTimedOut ||
      ad_event.type == ads::AdNotificationEventType::kDismissed) {
    UpdateArm(/* reward */ 0, segment);
  } else if (ad_event.type == ads::AdNotificationEventType::kClicked) {
...
void EpsilonGreedyBandit::UpdateArm(
  ...
  auto iter = std::find_if(arms.begin(), arms.end(),
      [&segment](EpsilonGreedyBanditArmInfo& arm) {
    return arm.segment == segment;
  });
  ...
  // TODO(Moritz Haller): better ideom, e.g.
  // EpsilonGreedyBanditArmInfo arm = *iter;
